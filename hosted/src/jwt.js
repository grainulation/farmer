...
export async function importPublicKeyJWK(jwk) {
  return crypto.subtle.importKey('jwk', { ...jwk, key_ops: ['verify'] }, ALG, false, ['verify']);
}
...
export async function sign(privateKey, payload) {
  ...
  const rawSig = await crypto.subtle.sign(SIGN_ALG, privateKey, enc.encode(signingInput));
...
export async function verify(publicKey, token) {
  ...
  const valid = await crypto.subtle.verify(SIGN_ALG, publicKey, signature, signingInput);
 ...
 */
export async function createSprintJWT(privateKey, sprintToken, role = 'admin', ttlSeconds = 86400) {
