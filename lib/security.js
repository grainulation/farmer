...
export const SECURITY_HEADERS = {
  "Content-Security-Policy":
    "default-src 'self'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; connect-src 'self' ws: wss:; frame-ancestors 'none'",
...
// --- Token management (triple-token: admin + viewer + hook) ---
export class TokenManager {
   ...
   * @param {string}  opts.hookToken - Initial hook-auth token (or auto-generated)
  ...
  constructor(opts = {}) {
    ...
    this._viewer = new RoleToken(
      opts.viewerToken || undefined,
      this.gracePeriod,
    );
    // Narrow-scope hook token — used only to authenticate /hooks/* POSTs from
    // local notifier scripts (e.g. grainulator's sprint-status-notifier.cjs).
    // Kept separate from admin/viewer so leaking the hook token can't be used
    // to impersonate a dashboard user.
    this._hook = new RoleToken(opts.hookToken || undefined, this.gracePeriod);
  ...
  /** The viewer token. */
  get viewerToken() {
    return this._viewer.token;
  }

  /** The hook-auth token. */
  get hookToken() {
    return this._hook.token;
  }

  /**
   * Check if a provided token matches the hook role specifically.
   * Hook tokens authenticate /hooks/* POSTs only — never dashboard access.
   * @returns {boolean}
   */
  matchesHook(provided) {
    if (!provided) return false;
    return this._hook.matches(provided);
  }
  ...
  rotate(broadcastFn) {
    ...
    console.log(
      `  [rotate] Admin token rotated. New: ${newToken.slice(0, 8)}...`,
    );
    if (broadcastFn) {
      broadcastFn({
        type: "token_rotated",
        data: { newToken, gracePeriod: this.gracePeriod },
      });
    }
    return newToken;
  }
  ...
  rotateViewer(broadcastFn) {
    ...
    console.log(
      `  [rotate] Viewer token rotated. New: ${newToken.slice(0, 8)}...`,
    );
    if (broadcastFn) {
      broadcastFn({
        type: "viewer_token_rotated",
  ...
  /** Serialize all tokens to JSON for persistence. */
  toJSON() {
    return {
      admin: this._admin.token,
      viewer: this._viewer.token,
      hook: this._hook.token,
    };
  }

  /**
   * Load tokens from persisted data.
   * Handles backwards compat: plain text = admin token, generate viewer + hook.
   * @param {string} raw - Raw file content (plain text or JSON)
   * @returns {{ admin: string, viewer: string, hook: string }}
   */
  static parseTokenFile(raw) {
    const trimmed = (raw || "").trim();
    if (!trimmed) return { admin: "", viewer: "", hook: "" };
    // Try JSON first
    ...
    // Plain text = old format, treat as admin token (hook token will be generated)
    return { admin: trimmed, viewer: "", hook: "" };
  }
...
export class PidLock {
      ...
      try {
        ...
        if (oldPid && oldPid !== process.pid) {
          try {
            ...
            throw new Error(
              `Another Farmer instance is running (PID ${oldPid}). Kill it first: kill ${oldPid}`,
            );
          } catch (e) {
  ...
  release() {
    try {
      if (
        existsSync(this.pidPath) &&
        readFileSync(this.pidPath, "utf8").trim() === String(process.pid)
      ) {
  ...
  /**
   * Force-release the PID lock regardless of ownership (for stop/cleanup commands).
   */
  forceRelease() {
    try {
