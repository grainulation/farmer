...
export class TokenManager {
  /**
   * @param {object} opts
   * @param {string}  opts.token - Initial token (or auto-generated)
  ...
  /** Rotate to a new token, keeping old one valid during grace period. */
  rotate(broadcastFn) {
    const oldToken = this.token;
    this.token = randomBytes(16).toString('hex');
    this.retiredTokens.push({ token: oldToken, retiredAt: Date.now() });
    this._purgeRetired();
    console.log(`  [rotate] Auth token rotated. New: ${this.token.slice(0, 8)}...`);
    if (broadcastFn) {
      broadcastFn({ type: 'token_rotated', data: { newToken: this.token, gracePeriod: this.gracePeriod } });
    }
    return this.token;
  }

  /** Check if a provided token matches current or retired (within grace). */
  matches(provided) {
    if (!provided) return false;
    // Check current token
    if (this._safeEqual(provided, this.token)) return true;
    // Check retired tokens within grace period
    const cutoff = Date.now() - this.gracePeriod * 1000;
    for (const rt of this.retiredTokens) {
      if (rt.retiredAt > cutoff && this._safeEqual(provided, rt.token)) return true;
    }
    return false;
  }
