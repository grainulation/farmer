...
export function buildCompactContext(cwd, ecosystemPorts) {
  ...
  const unresolvedConflicts = Array.isArray(conflictGraph.unresolved) ? conflictGraph.unresolved : [];
  ...
  for (const [name, info] of Object.entries(phase)) {
    lines.push(`  ${name}: ${info.claims} claims${info.complete ? ' [complete]' : ''}`);
  }

  lines.push('');
  lines.push(`COVERAGE: ${topics.length} topics | ${blockerTopics.length} blocked | ${weakTopics.length} weak`);
  ...
  if (unresolvedConflicts.length > 0) {
    ...
    for (const c of unresolvedConflicts.slice(0, 3)) {
      lines.push(`  - ${c.claim_a} vs ${c.claim_b}: ${c.reason || 'no reason'}`);
    }
  }
  ...
  lines.push('');
  lines.push(`Compilation status: ${compilation.status} | Errors: ${(compilation.errors || []).length} | Warnings: ${(compilation.warnings || []).length}`);
  lines.push(`== END SPRINT CONTEXT ==`);

  const text = lines.join('\n');
  const contextHash = createHash('md5').update(text).digest('hex').slice(0, 8);

  return { text, hash: contextHash, timestamp: now, compilationHash: compilation.claims_hash, stale: false };
}
...
function buildMinimalContext(cwd, now) {
  ...
  return { text, hash: 'minimal', timestamp: now, compilationHash: null, stale: true };
}
