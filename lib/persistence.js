...
export class Persistence {
   ...
   *
   * Rotation failures (EACCES on read-only ~/.farmer, EROFS on
   * Dropbox-synced home, etc.) used to be silently swallowed.
   * Now we surface the first failure per error code to stderr and
   * rate-limit the follow-ups so the log itself doesn't become a
   * spam vector. Unbounded audit-log growth is still possible on
   * truly read-only storage, but now the operator has a signal.
   */
  rotateIfNeeded() {
    try {
      ...
      for (let i = this.maxAuditFiles - 1; i >= 1; i--) {
        ...
        if (i === this.maxAuditFiles - 1) {
          try {
            ...
            this._reportRotateFailure("unlink-oldest", e);
          }
        }
        if (i === 1) {
          try {
            ...
            this._reportRotateFailure("rename-current", e);
          }
        } else {
          ...
          try {
            ...
            this._reportRotateFailure("rename-shift", e);
          }
        }
      }
  ...
  /**
   * Log a rotation failure once per (stage, code) pair, then rate-limit
   * the same combination to once per 5 minutes so a persistent condition
   * doesn't flood stderr. ENOENT on "unlink-oldest" is benign (first
   * rotation where the oldest file hasn't been created yet) — skipped.
   */
  _reportRotateFailure(stage, err) {
    if (stage === "unlink-oldest" && err.code === "ENOENT") return;
    this._rotateLogSeen ||= new Map();
    const key = `${stage}:${err.code || "UNKNOWN"}`;
    const now = Date.now();
    const last = this._rotateLogSeen.get(key) || 0;
    if (now - last < 5 * 60 * 1000) return;
    this._rotateLogSeen.set(key, now);
    process.stderr.write(
      `[audit] rotation ${stage} failed: ${err.code || err.name || "error"} — ${err.message}\n`,
    );
  }
