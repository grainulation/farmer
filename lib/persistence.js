...
import { existsSync, readFileSync, writeFileSync, renameSync, appendFileSync, statSync, unlinkSync } from 'node:fs';
...
export class Persistence {
  ...
  auditLog(entry) {
    try {
      ...
      const now = Date.now();
      if (this._auditWriteCount >= 100 || now - this._auditLastCheckTs >= 60_000) {
        this._auditWriteCount = 0;
  ...
  rotateIfNeeded() {
    try {
      ...
      for (let i = this.maxAuditFiles - 1; i >= 1; i--) {
        ...
        if (i === 1) {
          try { renameSync(this.auditLogPath, older); } catch {}
        } else {
          const newer = this.auditLogPath.replace('.jsonl', `.${i - 1}.jsonl`);
          try { renameSync(newer, older); } catch {}
        }
      }
