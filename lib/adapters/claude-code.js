...
export class ClaudeCodeAdapter extends BaseAdapter {
  ...
  formatResponse(decision, context) {
    const { hookEvent = 'PermissionRequest', isQuestion = false } = context || {};
    ...
    if (hookEvent === 'PreToolUse') {
      ...
      return {
        hookSpecificOutput: {
          ...
          permissionDecision: isQuestionResponse ? 'deny' : (decision.allow ? 'allow' : 'deny'),
    ...
    return {
      hookSpecificOutput: {
        ...
        decision: {
          ...
          message: decision.reason || (decision.allow ? 'Approved via Farmer' : 'Denied via Farmer'),
        }
      }
    };
  }
  ...
  parseNotification(body) {
    const prompt = body.tool_input?.prompt
