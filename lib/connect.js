...
// Strict exact-template for pre-1.1.5 detection (rt003). Any command that
// matches this regex is almost certainly farmer 1.1.4 — third-party tools
// would not coincidentally emit the exact same shape including the
// `--data-binary @- 2>/dev/null || true` tail. If a user hand-edited to add
// an Authorization header, it NO longer matches this regex and is preserved.
const LEGACY_1_1_4_REGEX =
  /^cat \| curl -s -X POST http:\/\/127\.0\.0\.1:\d+\/hooks\/(permission|activity|notification|stop) -H 'Content-Type: application\/json' --data-binary @- 2>\/dev\/null \|\| true$/;
