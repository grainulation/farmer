...
// Pre-1.1.5 detection (rt003 + blindspot #1). Matches:
//   (a) the verbatim 1.1.4 shape (`cat | curl -s -X POST ...`), AND
//   (b) 1.1.4 with a single `-H @<path>` inserted before Content-Type —
//       i.e. Aid's own hand-patch pattern, which any other 1.1.4 user
//       debugging the silent-401 bug would've arrived at.
// Both variants END with the exact 1.1.4 tail (`--data-binary @- 2>/dev/null || true`)
// which is what rules out legitimate third-party hooks on other ports.
// Also narrows the path allowlist to our 4 endpoints so a hook targeting
// /hooks/log or similar is still preserved.
const LEGACY_1_1_4_REGEX =
  /^cat \| curl -s(?:S)? -X POST http:\/\/127\.0\.0\.1:\d+\/hooks\/(permission|activity|notification|stop) (?:-H "?@\S+"? )?-H 'Content-Type: application\/json' --data-binary @- 2>\/dev\/null \|\| true$/;

// Shell-safe path charset. The emitted hook command interpolates the
// absolute path to hook-auth.header into a double-quoted curl arg; any
// char outside this set could escape the quoting (rt002). POSIX-portable
// chars that curl + absolute-path lookup actually need.
const SAFE_PATH_RE = /^[A-Za-z0-9/._\-~+:@,%]+$/;

/**
 * Refuse to emit a hook command if the auth-file path contains characters
 * that could escape the shell-string interpolation (rt002). Throwing here
 * is the right UX: whatever picked this dataDir (CLI flag, status probe)
 * is the bug; silently emitting an unsafe command would be CVE-grade.
 */
function assertShellSafePath(path) {
  if (!SAFE_PATH_RE.test(path)) {
    throw new Error(
      `Refusing to emit hook command: path contains unsafe characters for shell interpolation: ${JSON.stringify(path)}. ` +
        `Move farmer's data dir to a plain path (alphanumeric + /._-~+:@,% only).`,
    );
  }
}
 ...
 *   - Path is charset-whitelisted to prevent shell-injection via dataDir
 *     (rt002) — attempt to emit with a hostile path throws before write.
 */
export function buildHookCommand(base, path, hookAuthPath) {
  let authArg = "";
  if (hookAuthPath) {
    assertShellSafePath(hookAuthPath);
    authArg = `-H "@${hookAuthPath}" `;
  }
  return (
 ...
 * or (b) the exact 1.1.4 template verbatim (rt003 fix — the prior "prefix
 * match" dragnet would rewrite any third-party hook whose command happened
 * to start `cat | curl -s -X POST http://127.0.0.1:...`).
 *
 * Anything outside those two branches is left alone: hand-edits, other
 * tools, and any future shape farmer might emit that we haven't taught
 * this function to recognize.
 */
  ...
  return hookEntry.hooks.some(
    (h) =>
      ...
      (h.command.includes(FARMER_SENTINEL) ||
        LEGACY_1_1_4_REGEX.test(h.command)),
  );
}

/**
 * Distinguishes "current shape" from "stale shape that needs migration".
 * A hook is current iff it carries the 1.1.5+ sentinel; anything matching
 * only the legacy regex is stale and must be rewritten. Blindspot #1:
 * without this, `farmer connect` saw the hand-patched 1.1.4 hook as
 * "already connected" (port matched) and exited without migrating.
 */
function isStaleFarmerHook(hookEntry) {
  if (!hookEntry?.hooks) return false;
  const cmds = hookEntry.hooks.map((h) => h.command || "");
  if (cmds.some((c) => c.includes(FARMER_SENTINEL))) return false;
  return cmds.some((c) => LEGACY_1_1_4_REGEX.test(c));
}
...
/**
 * Atomically write JSON with 2-space indent (rt008). Uses tmp+rename so
 * a crash mid-write can't corrupt the destination (important for
 * `~/.claude/settings.json` — Claude Code refuses to start if that file
  ...
  if (isGlobal) {
    ...
    if (existing.hooks) {
      ...
      if (alreadyHasFarmer) {
        ...
        const hasStale = Object.values(existing.hooks).some((entries) =>
          (entries || []).some(isStaleFarmerHook),
        );
        if ((existingPort && existingPort !== port) || hasStale) {
