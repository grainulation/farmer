...
function newTunnel() {
  ...
  fetch('/api/new-tunnel', { method: 'POST' })
    ...
    .then(data => {
      ...
      // Re-draw admin QR with tunnel URL (viewer QR needs separate handling)
      drawQR('qr-admin', data.url);
      document.getElementById('admin-link').href = data.url;
      document.getElementById('admin-link').textContent = data.url;
