...
${ROUTES.map((r) => "<tr><td><code>" + r.method + "</code></td><td><code>" + r.path + "</code></td><td>" + r.description + "</td></tr>").join("")}
    ...
    if (req.method === "POST" && url.pathname === "/api/feedback") {
      ...
      // Optional session targeting — if provided, feedback is scoped to that session
    ...
    // Optional query: ?session_id=xxx to only return feedback targeted at that session
    if (req.method === "GET" && url.pathname === "/api/feedback/poll") {
      ...
      const filterSessionId = url.searchParams.get("session_id") || null;
      ...
      for (const item of this._feedbackQueue) {
        ...
        // Session filtering: if query specifies session_id, return items
        // targeted at that session OR untargeted items (session_id === null).
        // If no query filter, return all items (backward compatible).
        if (filterSessionId) {
          if (item.session_id && item.session_id !== filterSessionId) continue;
        }
        const stalePrefix =
  ...
  _consumePendingFeedback(sessionId) {
    const pending = this._feedbackQueue.filter((f) => {
      if (f.status !== "pending") return false;
      if (!sessionId) return true;
      return f.session_id == null || f.session_id === sessionId;
    });
