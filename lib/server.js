...
const ROUTES = [
  ...
  },
  {
    ...
    description: "Hook: sprint status push from grainulator",
  },
  {
    method: "POST",
    path: "/api/decide",
  ...
  },
  {
    ...
    path: "/api/feedback",
    description:
      "Submit feedback to the queue (optional session_id for targeting)",
  },
  {
    method: "GET",
    path: "/api/feedback/poll",
    description:
      "Poll pending feedback items (localhost only, optional ?session_id filter)",
  },
  {
    method: "POST",
...
export class FarmerServer {
  constructor(opts = {}) {
    ...
    let hookToken = opts.hookToken || "";
    // Tracks whether the on-disk file carried a hook token when the server
    // started. Used by the opportunistic hook-auth path: if absent, we let
    // unauthenticated loopback POSTs through with a one-time stderr warning
    // so pre-upgrade users don't get hard-failed by the new auth check.
    this._hookTokenPresentAtStartup = false;
    // Always attempt to read the persisted token file so we can pick up the
    // viewer / hook fields even when the CLI supplied an admin token via
    // --token. Without this, tests and ops scripts that pass --token could
    // never flip farmer into hook-auth enforcement mode.
    try {
      const raw = readFileSync(tokenPath, "utf8");
      const parsed = TokenManager.parseTokenFile(raw);
      if (!adminToken) adminToken = parsed.admin;
      viewerToken = viewerToken || parsed.viewer;
      hookToken = hookToken || parsed.hook;
      if (parsed.hook) this._hookTokenPresentAtStartup = true;
    } catch (e) {
      vlog("parseToken", e.message);
    }
    this.tokenManager = new TokenManager({
      token: adminToken || undefined,
      viewerToken: viewerToken || undefined,
      hookToken: hookToken || undefined,
    ...
    try {
      ...
      vlog("persistToken", e.message);
    }
    // One-time stderr warning when /hooks/* requests arrive before a hook
    // token has been provisioned on disk. We guard with a flag so heavy
    // hook traffic doesn't spam the log.
    this._hookAuthWarned = false;

    // Hook statistics — exposed via GET /status for `farmer status` and
    // the connect-server seam e2e test. Also drives the user-visible
    // warning when /hooks/* returns 401s (see .farmer-broken-installs.jsonl).
    this._hookStats = {
      received: 0,
      accepted: 0,
      rejected401: 0,
    ...
    this._hookAuthPath = join(this.dataDir, "hook-auth.header");
    this._brokenInstallsPath = join(
      this.dataDir,
      ".farmer-broken-installs.jsonl",
    );

    // If hook auth is enforced, materialize the auth header file so hook
    // curls can pass `-H @<path>` without ever putting the token on argv.
    // Atomic write (tmp + rename) so concurrent mid-flight curl reads never
    // see a truncated file.
    this._writeHookAuthFile();
  ...
  start() {
    ...
    this.server.listen(this.port, "127.0.0.1", () => {
      ...
      console.log(
        `  Hook:    ${this.tokenManager.hookToken}  (for /hooks/* Bearer auth)`,
      );
      console.log(`  Hooks:  /hooks/permission  /hooks/activity`);
      console.log(
        `          /hooks/notification /hooks/lifecycle /hooks/stop /hooks/sprint-status`,
      );
  ...
  _getSession(sessionId, cwd, req) {
    ...
    if (!this.sessions.has(id)) {
      ...
      // rt005: pid-based fingerprint on loopback is client-controlled (all
      // local pids are visible via `ps`), so binding does NOT enforce per-
      // session isolation on 127.0.0.1. Rather than pretend, we skip the
      // binding for loopback and document the limitation in SECURITY.md.
      // Per-session tokens are a 1.2.0 roadmap item.
      const addr = req ? clientAddr(req, this.trustProxy) : null;
      const isLoopback =
        addr === "127.0.0.1" || addr === "::1" || addr === "::ffff:127.0.0.1";
      const fp =
        req && !isLoopback ? sourceFingerprint(addr, req._hookPid) : null;
      ...
      // rt005: only enforce binding for non-loopback callers. Loopback pid
      // is client-controlled (visible via ps), so the binding is security
      // theater for 127.0.0.1 and triggers false-positive mismatches when
      // Claude Code restarts with a new pid but reuses a session_id.
      const addr = clientAddr(req, this.trustProxy);
      const isLoopback =
        addr === "127.0.0.1" || addr === "::1" || addr === "::ffff:127.0.0.1";
      if (!isLoopback) {
        const fp = sourceFingerprint(addr, req._hookPid);
  ...
  // --- Strict scope guard for file-read features (rt004 re-verify) ---
  //
  // Unlike `_isRegisteredProject`, this does NOT default-allow on an empty
  // list. The re-verify red-team confirmed that the permission auto-allow
  // path needs empty-list fallback (workflow trust), but /hooks/lifecycle
  // compact-context READS arbitrary files from `lc.cwd` and echoes them
  // back — that is an exfiltration surface that must be explicitly opted
  // into via project registration. Default install has no registered
  // projects, so compact-context is disabled by default; users enable it
  // per-project via `farmer connect` (which registers the cwd).
  _canReadCompactContextFor(cwd) {
    if (!cwd) return false;
    if (this.registeredProjects.length === 0) return false;
    const resolved = resolve(cwd);
    return this.registeredProjects.some(
      (p) => resolved === p || resolved.startsWith(p + "/"),
    );
  ...
  // --- Hook-auth header file (bs-22 / 1.1.5) ---
  //
  // When hook-auth is enforced, farmer writes the current hook token into
  // `<dataDir>/hook-auth.header` as a single line (`Authorization: Bearer <t>`)
  // with mode 0600. `farmer connect` embeds the absolute path into the hook
  // curls (`-H @<path>`), so the token never hits argv and rotates in place.
  //
  // Hardening (rt001): the tmp file gets a randomBytes-suffixed name and
  // is opened with O_EXCL|O_CREAT so a pre-staged symlink at the tmp path
  // can't hijack the token write. `rename()` is atomic on POSIX and swaps
  // the entry inode-level, so concurrent `-H @file` curls never read a
  // partial write.
  _writeHookAuthFile() {
    if (!this._hookTokenPresentAtStartup) return;
    const token = this.tokenManager.hookToken;
    if (!token) return;
    const tmp = `${this._hookAuthPath}.${randomBytes(8).toString("hex")}.tmp`;
    let fd = null;
    try {
      // O_WRONLY (1) | O_CREAT (64) | O_EXCL (128) = 193 on Linux/macOS.
      // Fails with EEXIST if `tmp` exists — symlink hijack becomes impossible.
      fd = openSync(tmp, O_WRONLY | O_CREAT | O_EXCL, 0o600);
      writeSync(fd, `Authorization: Bearer ${token}\n`);
      closeSync(fd);
      fd = null;
      renameSync(tmp, this._hookAuthPath);
    } catch (e) {
      vlog("writeHookAuthFile", e.message);
      if (fd !== null) {
        try {
          closeSync(fd);
        } catch {}
      }
      // Best-effort cleanup of a dangling tmp
      try {
        unlinkSync(tmp);
      } catch {}
    }
  ...
  _recordHook401(type, remote) {
    this._hookStats.rejected401++;
    // rt007: debounce per (type, remote) so a 5800 req/s 401 flood can't
    // bloat .farmer-broken-installs.jsonl into gigabytes. We still want
    // the FIRST failure of a given shape to log — that's the user-visible
    // diagnostic signal. Subsequent dupes within DEBOUNCE_MS are dropped.
    if (!this._brokenInstallsDebounce) this._brokenInstallsDebounce = new Map();
    const DEBOUNCE_MS = 60_000;
    const key = `${type}|${remote}`;
    const now = Date.now();
    const last = this._brokenInstallsDebounce.get(key) || 0;
    if (now - last < DEBOUNCE_MS) return;
    this._brokenInstallsDebounce.set(key, now);
    // GC old entries opportunistically so the Map doesn't grow unbounded
    // across a week of mixed clients.
    if (this._brokenInstallsDebounce.size > 1000) {
      for (const [k, t] of this._brokenInstallsDebounce) {
        if (now - t > DEBOUNCE_MS) this._brokenInstallsDebounce.delete(k);
      }
    }
    try {
      const line =
        JSON.stringify({
          ...
          type,
          remote,
          reason: "missing_or_wrong_bearer",
        }) + "\n";
      appendFileSync(this._brokenInstallsPath, line, { mode: 0o600 });
    } catch (e) {
      vlog("recordHook401", e.message);
    }
  }

  _recordHookAccepted(type) {
    this._hookStats.accepted++;
    this._hookStats.lastAt = new Date().toISOString();
    this._hookStats.lastType = type;
  }
  ...
  _loadFeedbackQueue() {
    try {
      ...
      // Blindspot#2: expire any PENDING items older than the 30-minute
      // EXPIRE_MS window. rt-fb4 purges on live session_end, but items
      // orphaned across a restart (common after a farmer crash or an
      // upgrade that drops in-memory sessions) would otherwise sit pending
      // forever and get delivered to the first unlucky new session.
      const now = Date.now();
      const EXPIRE_MS = 30 * 60 * 1000;
      let expired = 0;
      for (const item of queue) {
        if (item.status === "pending" && now - item.timestamp > EXPIRE_MS) {
          item.status = "expired";
          item.expired_at = now;
          item.expire_reason = "startup_orphan";
          expired++;
        }
      }
      if (expired > 0) {
        process.stderr.write(
          `[farmer] expired ${expired} orphaned feedback item(s) at startup (>30min pending)\n`,
        );
      }
      return queue;
    } catch (e) {
      vlog("loadMessages", e.message);
    }
    return [];
  }

  // Blindspot#3: writeFileSync is not atomic — a crash mid-write can
  // corrupt .farmer-feedback and brick the queue load. Same class as
  // rt008. Use tmp+rename for durability.
  _persistFeedbackQueue() {
    const tmp = `${this._feedbackQueuePath}.${randomBytes(6).toString("hex")}.tmp`;
    try {
      writeFileSync(tmp, JSON.stringify(this._feedbackQueue), {
        encoding: "utf8",
      });
      renameSync(tmp, this._feedbackQueuePath);
    } catch (e) {
      vlog("feedbackQueueFlush", e.message);
      try {
        unlinkSync(tmp);
      } catch {}
    }
  ...
  async _handleRequest(req, res) {
    ...
    // rt006: DNS-rebinding guard. Farmer binds loopback-only; a browser on
    // evil.com that rebinds its DNS to 127.0.0.1 still sends Host: evil.com
    // on the rebound request (browsers do not update Host after rebind).
    // Reject anything whose Host header doesn't name loopback.
    ...
    if (!hostAllowed) {
      ...
      res.end(JSON.stringify({ error: "misdirected_host" }));
      return;
    }

    // CORS — pin to loopback only. Reflecting an arbitrary Origin with
    // Allow-Credentials:true was a DNS-rebinding amplifier in 1.1.4 (rt006).
    ...
    // --- Public health/status: unauth, loopback-only ---
    //
    // /status exposes hook counters + hookAuthMode + dataDir so:
    //   (a) `farmer status` CLI can render "N hook 401s — run farmer connect"
    //   (b) `farmer connect` can query the live server to discover its
    //       actual dataDir and embed the correct absolute path to
    //       hook-auth.header in the emitted hook curls. (bs-22 x003)
    //   (c) the connect<->server seam e2e test can assert acceptance.
    //
    // Loopback-only because it leaks internal paths/ports. No auth because
    // hooks themselves are unauth on fresh installs and we need connect to
    // read it before authenticating anything.
    if (req.method === "GET" && url.pathname === "/status") {
      ...
      res.end(
        JSON.stringify({
          ...
          hookAuthMode: this._hookTokenPresentAtStartup
            ? "enforced"
            : "opportunistic",
          hookAuthPath: this._hookTokenPresentAtStartup
            ? this._hookAuthPath
            : null,
          hooks: this._hookStats,
...
${ROUTES.map((r) => "<tr><td><code>" + r.method + "</code></td><td><code>" + r.path + "</code></td><td>" + r.description + "</td></tr>").join("")}
    ...
    if (req.method === "GET" && url.pathname === "/events") {
      ...
      if (this.sprintStatusData)
        res.write(
          `data: ${JSON.stringify({ type: "sprint_status", data: this.sprintStatusData })}\n\n`,
        );
    ...
    // --- Hook endpoints — opportunistic Bearer auth ---
    //
    // Shared-host risk (bs-19): without any auth, any local process on a
    // multi-user box (e.g. Citrix / shared Linux) can forge POSTs to /hooks/*
    // and pollute another user's dashboard. To fix without breaking pre-
    // upgrade users whose hook notifier doesn't yet send a token:
    //
    //   - If a hook token exists on disk at server start, every /hooks/* POST
    //     MUST carry "Authorization: Bearer <hookToken>" — no header, wrong
    //     header, or non-Bearer scheme => 401.
    //   - If no hook token was on disk at startup (first run, pre-upgrade),
    //     we log a one-time stderr warning and accept loopback POSTs so
    //     existing users don't see their hooks silently stop working.
    //
    // The admin/viewer tokens are intentionally NOT accepted here — a leaked
    // hook token must not grant dashboard access, and vice versa. Narrow
    // scope on both sides.
    if (req.method === "POST" && url.pathname.startsWith("/hooks/")) {
      ...
      const hookAuthHeader = req.headers["authorization"] || "";
      const providedToken = hookAuthHeader.startsWith("Bearer ")
        ? hookAuthHeader.slice(7)
        : "";

      const hookType = url.pathname.split("/hooks/")[1] || "unknown";
      this._hookStats.received++;

      if (this._hookTokenPresentAtStartup) {
        // Enforcement mode: hook token on disk => Bearer header is mandatory.
        if (!this.tokenManager.matchesHook(providedToken)) {
          this._recordHook401(hookType, remoteAddr);
          res.writeHead(401, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: "unauthorized" }));
          return;
        }
      } else {
        // Opportunistic mode: no token file => loopback-only + stderr warn.
        if (!isLocalhost) {
          res.writeHead(403, { "Content-Type": "application/json" });
          res.end(
            JSON.stringify({
              error: "forbidden",
              message: "hook endpoints require loopback when unauthenticated",
            }),
          );
          return;
        }
        if (!this._hookAuthWarned) {
          this._hookAuthWarned = true;
          process.stderr.write(
            "[farmer] hook auth disabled — create .farmer-token to enable Bearer auth on /hooks/* (see README)\n",
          );
        }
      }
      let body;
      ...
      // hookType was declared up-top so the 401 path can use it too
      this._recordHookAccepted(hookType);
      ...
      if (hookType === "sprint-status")
        return this._handleSprintStatus(data, res);
      res.writeHead(404);
    ...
    if (req.method === "POST" && url.pathname === "/api/feedback") {
      ...
      // rt-fb1: require session targeting. The previous "optional" design
      // queued untargeted feedback with session_id=null; any polling session
      // then received it (line 1672-1674 used to include null-targeted in
      // every poll). That silently flattened per-session isolation and let
      // feedback cross sessions by default. Now the UI must pick a session.
      ...
      if (!sessionId || typeof sessionId !== "string") {
        ...
        res.end(
          JSON.stringify({
            ...
            message:
              "Feedback must target a specific session. Broadcast/null-targeted feedback was removed in 1.1.5 to prevent cross-session leaks.",
          }),
        );
        return;
      }
      if (!this.sessions.has(sessionId)) {
        res.writeHead(400, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Unknown session_id" }));
        return;
      }
      ...
      this._broadcast({
        type: "user_feedback",
        data: { text, timestamp: Date.now(), session_id: sessionId },
      });
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    // GET /api/feedback/poll — Claude's dedicated feedback pipeline.
    // rt-fb2: now requires session_id query AND hook-token Bearer auth
    // (same enforcement as /hooks/*). Loopback alone is not a security
    // boundary on shared-UID hosts; a local process with the hook token
    // is the only caller that should read feedback addressed to a session,
    // and the session_id MUST match the caller's claim (no null-fallback).
    if (req.method === "GET" && url.pathname === "/api/feedback/poll") {
      ...
      // Require hook-token Bearer auth when enforcement is on. Skipped in
      // opportunistic mode for pre-1.1 compat (same pattern as /hooks/*).
      if (this._hookTokenPresentAtStartup) {
        const h = req.headers["authorization"] || "";
        const t = h.startsWith("Bearer ") ? h.slice(7) : "";
        if (!this.tokenManager.matchesHook(t)) {
          res.writeHead(401, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ error: "unauthorized" }));
          return;
        }
      ...
      // rt-fb1: poll MUST specify session_id. Untargeted items no longer
      // exist (submit-side rejects them), so the old "null-fallback"
      // poll behavior has nothing to return and is removed.
      ...
      if (!filterSessionId) {
        ...
        res.end(
          JSON.stringify({
            error: "session_id_required",
            message: "Feedback poll must specify session_id in query.",
          }),
        );
        return;
      }
      ...
      for (const item of this._feedbackQueue) {
        ...
        if (item.session_id !== filterSessionId) continue;
        const stalePrefix =
    ...
    // POST /api/feedback/ack — mark feedback items as delivered.
    // rt-fb2: same hook-token Bearer enforcement as /poll. An attacker
    // who can forge ack could silence feedback meant for another session.
    if (req.method === "POST" && url.pathname === "/api/feedback/ack") {
      ...
      // rt-fb1: ack must be scoped to a specific session. Without this,
      // a caller could ack items from any session by guessing ids. Caller
      // MUST declare which session they're acking on behalf of; any id
      // not matching that session is silently ignored (not errored,
      // because races with expiration or missing ids shouldn't DoS the
      // poll loop).
      const ackSessionId = data.session_id;
      if (!ackSessionId || typeof ackSessionId !== "string") {
        res.writeHead(400, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({
            error: "session_id_required",
            message: "Ack must declare the session_id the items belong to.",
          }),
        );
        return;
      }
    ...
    // POST /api/feedback/read — REMOVED in 1.1.5 (rt-fb3).
    //
    // The legacy endpoint marked ALL pending feedback as delivered
    // regardless of session, which silently broke per-session isolation:
    // any loopback caller could ack items belonging to any session. Now
    // 410 Gone with a migration pointer to /api/feedback/ack which is
    // scoped by session_id. Kept at this URL so existing callers see a
    // loud failure rather than silently relying on the broken semantics.
    if (req.method === "POST" && url.pathname === "/api/feedback/read") {
      ...
      res.end(
        JSON.stringify({
          ...
          message:
            "/api/feedback/read was removed in farmer 1.1.5 — it marked ALL pending feedback delivered regardless of target session. Use POST /api/feedback/ack with {session_id, ids} instead (per-session scope).",
          replacement: "/api/feedback/ack",
        }),
      );
      return;
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/rotate-token") {
      ...
      // E2E red-team P1: the prior shape rotated tokens in memory but
      // never persisted them to .farmer-token, so a restart resurrected
      // the pre-rotate token. Write the new token set atomically so
      // rotation is durable across restarts.
      try {
        const tmp = `${this._tokenPath}.${randomBytes(6).toString("hex")}.tmp`;
        writeFileSync(tmp, JSON.stringify(this.tokenManager.toJSON()), {
          mode: 0o600,
        });
        renameSync(tmp, this._tokenPath);
      } catch (e) {
        vlog("persistRotatedToken", e.message);
      }
      // If the rotation touched the hook token, rewrite hook-auth.header
      // atomically so in-flight hook curls pick up the new value on next
      // invocation. No-op if hook token didn't change.
      this._writeHookAuthFile();
      res.writeHead(200, {
        "Content-Type": "application/json",
    ...
    if (req.method === "GET" && url.pathname === "/api/ecosystem") {
      ...
      if (orchardPath) {
        try {
          ...
          vlog("parseOrchardJSON", e.message);
        }
      }
  ...
  _handlePermission(data, res, req) {
    ...
    if (shouldAutoApprove(session, toolName, toolInput)) {
      ...
      const pendingFeedback = this._consumePendingFeedback(session.id);
      let responseBody;
  ...
  _handleLifecycle(data, res, req) {
    ...
    if (lc.event === "session_start") {
      ...
      if (lc.source === "compact") {
        // rt004 (re-verify): _buildCompactContext reads files from `lc.cwd`
        // and echoes them back in the response + SSE broadcast. Use the
        // STRICT scope guard (`_canReadCompactContextFor`), not the
        // permission-path `_isRegisteredProject` — the latter default-
        // allows on empty registeredProjects for workflow trust, which
        // leaves the exfil hole open on default installs. Strict guard
        // refuses unless the cwd is explicitly registered.
        if (!this._canReadCompactContextFor(lc.cwd)) {
          console.warn(
            `[security] lifecycle compact refused: cwd "${lc.cwd}" not a registered project`,
          );
          this.persistence.auditLog({
            event: "compact_context_refused",
            session_id: lc.sessionId,
            cwd: lc.cwd,
  ...
  _processSessionEnd(data, session) {
    ...
    // E2E red-team P1: purge any pending feedback addressed to this
    // session so it can't re-attach to a new session that reuses the id.
    this._purgeFeedbackForSession(session.id);
    this._broadcast({
      type: "session_end",
      session_id: session.id,
  ...
  // --- Sprint status hook (grainulator -> farmer) ---
  _handleSprintStatus(data, res) {
    const sprint = data.sprint;
    if (!sprint || typeof sprint !== "object") {
      res.writeHead(400, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({ error: "Missing required field: sprint (object)" }),
      );
      return;
    }
    const toInt = (v) => {
    ...
    const payload = {
      ...
      sprint: {
        question: String(sprint.question || ""),
        claim_count: toInt(sprint.claim_count),
        phase: String(sprint.phase || "unknown"),
        conflict_count: toInt(sprint.conflict_count),
        compile_status: String(sprint.compile_status || "unknown"),
        certificate: sprint.certificate ? String(sprint.certificate) : null,
        timestamp: toInt(sprint.timestamp) || Date.now(),
      },
    };
    this.sprintStatusData = payload;
    this.persistence.auditLog({
      event: "sprint_status",
      session_id: payload.session_id,
      phase: payload.sprint.phase,
      claim_count: payload.sprint.claim_count,
      compile_status: payload.sprint.compile_status,
    });
    this._broadcast({ type: "sprint_status", data: payload });
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end('{"ok":true}');
  }
  ...
  _loadCompilation() {
    ...
    try {
      ...
      vlog("loadCompilation", e.message);
    }
  }
  ...
  _loadSprints() {
    ...
    try {
      ...
      vlog("loadSprints", e.message);
    }
  }

  // --- Feedback injection (consumes oldest pending item for auto-approve piggyback) ---
  // If sessionId is provided, only items matching that session_id (or untargeted/null) are consumed.
  // E2E red-team P0: previous shape returned ANY pending item when
  // sessionId was missing, plus null-targeted items for any session.
  // Both patterns leak feedback across sessions. Now: strict match on
  // session_id; missing caller id → no consumption.
  _consumePendingFeedback(sessionId) {
    if (!sessionId) return null;
    const item = this._feedbackQueue.find(
      (f) => f.status === "pending" && f.session_id === sessionId,
    );
    if (!item) return null;
    item.status = "delivered";
  ...
  // E2E red-team P1: feedback queued for a session outlived session_end
  // and re-attached when a new session reused the id. Purge on session
  // end so the queue and the session lifecycle stay consistent.
  _purgeFeedbackForSession(sessionId) {
    if (!sessionId) return 0;
    let purged = 0;
    for (const item of this._feedbackQueue) {
      if (item.status === "pending" && item.session_id === sessionId) {
        item.status = "expired";
        item.expired_at = Date.now();
        item.expire_reason = "session_ended";
        purged++;
      }
    }
    if (purged > 0) this._persistFeedbackQueue();
    return purged;
  }
