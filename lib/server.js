...
const ROUTES = [
  ...
  { method: 'POST', path: '/hooks/stop', description: 'Hook: stop event from AI agent' },
  ...
  { method: 'GET', path: '/api/admin/invite', description: 'Generate a signed invite URL for a role' },
  { method: 'GET', path: '/invite', description: 'Validate invite link and set auth cookie' },
...
export class FarmerServer {
  constructor(opts = {}) {
    ...
    try { writeFileSync(tokenPath, JSON.stringify(this.tokenManager.toJSON()), { mode: 0o600 }); } catch {}
...
${ROUTES.map(r => '<tr><td><code>'+r.method+'</code></td><td><code>'+r.path+'</code></td><td>'+r.description+'</td></tr>').join('')}
    ...
    if (req.method === 'GET' && url.pathname === '/events') {
      const auth = roleAuth();
      if (!auth.authenticated) { res.writeHead(401); res.end('Unauthorized'); return; }
      res.writeHead(200, {
      ...
      res.write(`data: ${JSON.stringify({ type: 'init', data: { ...this._initPayload(csrfToken), role: auth.role } })}\n\n`);
      ...
      vlog('sse', `${auth.role} client connected (${this.sseClients.size} total)`);
    ...
    if (req.method === 'POST' && url.pathname.startsWith('/hooks/')) {
      ...
      const isRequestTool = data.tool_name === 'AskUserQuestion' || data.tool_name === 'Request';
      if (hookType === 'permission' && !isRequestTool && !this._isRegisteredProject(data.cwd)) {
        ...
        let responseBody;
        responseBody = { hookSpecificOutput: { hookEventName: hookEvent, permissionDecision: 'allow', permissionDecisionReason: reason } };
    ...
    if (req.method === 'POST' && url.pathname === '/api/decide') {
      const auth = roleAuth();
      if (!auth.authenticated) { res.writeHead(401); res.end('Unauthorized'); return; }
      if (auth.role !== 'admin') { res.writeHead(403, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: 'Viewer role cannot perform this action' })); return; }
    ...
    if (req.method === 'GET' && url.pathname === '/api/claims') {
      if (!roleAuth().authenticated) { res.writeHead(401); res.end('Unauthorized'); return; }
      res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (req.method === 'GET' && url.pathname === '/connect') {
      ...
      const auth = roleAuth();
      if (!auth.authenticated || auth.role !== 'admin') { res.writeHead(404, { 'Content-Type': 'text/html' }); res.end('<html><body><h1>404</h1><p>Not Found</p></body></html>'); return; }
    ...
    if (req.method === 'POST' && url.pathname === '/api/admin/rotate-token') {
      ...
      if (auth.role !== 'admin') { res.writeHead(403, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: 'Viewer role cannot perform this action' })); return; }
      if (!csrfOk()) { res.writeHead(403); res.end('CSRF token invalid'); return; }
      const newToken = this.tokenManager.rotate((msg) => this._broadcast(msg, 'admin'));
      res.writeHead(200, {
    ...
    if (req.method === 'GET' && url.pathname === '/invite') {
      ...
      if (!inviteRole || !exp || !sig) { res.writeHead(400); res.end('Invalid invite link'); return; }
      if (Date.now() > exp) { res.writeHead(410); res.end('Invite link expired'); return; }
      if (!this.tokenManager.verifyInvite(inviteRole, exp, sig)) { res.writeHead(403); res.end('Invalid invite signature'); return; }
      const token = inviteRole === 'admin' ? this.tokenManager.token : this.tokenManager.viewerToken;
      res.writeHead(302, {
        'Set-Cookie': [
    ...
    if (req.method === 'GET' && url.pathname === '/api/admin/invite') {
      ...
      if (!['admin', 'viewer'].includes(inviteRole)) { res.writeHead(400, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: 'Invalid role' })); return; }
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ url: inviteUrl, role: inviteRole, expiresAt: exp }));
      return;
    }
    ...
    if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/index.html' || url.pathname === '/desktop' || url.pathname === '/m' || url.pathname === '/mobile')) {
      ...
      const urlToken = url.searchParams.get('token');
      if (urlToken) {
        const tokenToSet = auth.role === 'admin' ? this.tokenManager.token : this.tokenManager.viewerToken;
        res.writeHead(302, {
          'Set-Cookie': [
      ...
      res.end(this._dashboardPage(auth.role)); return;
    }
  ...
  _handlePermission(data, res, req) {
    ...
    if (this.sseClients.size === 0 && !isQuestion && toolName !== 'AskUserQuestion' && toolName !== 'Request') {
  ...
  _handleRules(data, res) {
    ...
    const ruleList = Array.isArray(data.rules) ? data.rules : (data.rule ? [data.rule] : []);
    if (ruleList.length === 0 || !ruleList.every(r => r.tool)) {
      res.writeHead(400); res.end(JSON.stringify({ error: 'rule.tool required' })); return;
    }
...
<input id="t" type="password" placeholder="Token" autofocus><button type="submit">Connect</button></form></div></body></html>`;
  ...
  _dashboardPage(role = 'admin') {
    ...
    try {
      ...
      const token = role === 'admin' ? this.tokenManager.token : this.tokenManager.viewerToken;
