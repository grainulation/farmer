...
const ROUTES = [
  ...
  { method: 'POST', path: '/api/feedback', description: 'Submit feedback on a decision' },
  { method: 'POST', path: '/api/feedback/read', description: 'Mark feedback items as read' },
    ...
    this.server.listen(this.port, '127.0.0.1', () => {
      ...
      if (!this.noTunnel) {
        this._startTunnel((err, publicUrl) => {
          ...
          if (!this.noOpen) this._openBrowser(connectPage);
        });
      } else {
  ...
  _handleWsText(client, payload) {
    ...
    if (!client.authenticated) {
      ...
      if (this.tokenManager.matches(token)) {
        ...
        if (this.claimsData) this._wsSend(client, { type: 'claims', data: this.claimsData });
        if (this.compilationData) this._wsSend(client, { type: 'compilation', data: this.compilationData });
        if (this.sprintsData.length) this._wsSend(client, { type: 'sprints_list', data: this.sprintsData });
      } else {
  ...
  _consumePendingFeedback() {
    ...
    try {
      ...
      if (fb && !fb.read && fb.text) {
        fb.read = true;
        writeFileSync(fbPath, JSON.stringify(fb), 'utf8');
        this._broadcast({ type: 'feedback_read', data: {} });
        return fb.text;
      }
    } catch {}
...
const qr = QRC.encodeText("${connectUrl}", QRC.Ecc.LOW);
