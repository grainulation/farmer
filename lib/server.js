...
export class FarmerServer {
  ...
  //
  // Atomic write: tmp + rename. Guarantees concurrent mid-flight curl reads
  // never see an empty/truncated file during rotation.
  _writeHookAuthFile() {
    if (!this._hookTokenPresentAtStartup) return;
    const token = this.tokenManager.hookToken;
    if (!token) return;
    const tmp = this._hookAuthPath + ".tmp";
    try {
      writeFileSync(tmp, `Authorization: Bearer ${token}\n`, { mode: 0o600 });
