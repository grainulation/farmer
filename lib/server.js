...
${ROUTES.map((r) => "<tr><td><code>" + r.method + "</code></td><td><code>" + r.path + "</code></td><td>" + r.description + "</td></tr>").join("")}
    ...
    // --- Hook endpoints (localhost or Bearer token) ---
    if (req.method === "POST" && url.pathname.startsWith("/hooks/")) {
      ...
      if (hookAuthHeader.startsWith("Bearer ")) {
        const role = this.tokenManager.matches(hookAuthHeader.slice(7));
        if (role) {
          hookAuthorized = true;
        } else {
          res.writeHead(401);
      ...
      if (!hookAuthorized) {
        res.writeHead(403);
        res.end("Hook endpoints require localhost or valid Bearer token");
        return;
      }
      let body;
      try {
