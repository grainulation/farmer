  ...
  _handleLifecycle(data, res, req) {
    ...
    if (lc.event === "session_start") {
      ...
      if (lc.source === "compact") {
        // rt004: _buildCompactContext reads files from `lc.cwd` and echoes
        // them back in the response + SSE broadcast. Without a scope check,
        // any authenticated hook client can POST cwd=<someone else's path>
        // and exfiltrate that directory's compilation.json. Require the
        // cwd to be a registered project (same gate the /hooks/permission
        // path uses for auto-allow). Unregistered cwds get an empty
        // additionalContext — no file read, no broadcast.
        if (!this._isRegisteredProject(lc.cwd)) {
          console.warn(
