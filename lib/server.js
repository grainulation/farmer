...
import { readFileSync, writeFileSync, existsSync, watchFile, readdirSync } from 'node:fs';
...
export class FarmerServer {
  constructor(opts = {}) {
    ...
    // Optional claims integration (wheat sprint data)
  ...
  start() {
    ...
    // Load claims if configured
    this._loadClaims();
    if (this.claimsPath && existsSync(this.claimsPath)) {
      watchFile(this.claimsPath, { interval: 2000 }, () => {
        this._loadClaims();
        this._broadcast({ type: 'claims', data: { claims: this.claimsData, compilation: this.compilationData } });
      });
    }
    if (this.compilationPath && existsSync(this.compilationPath)) {
      watchFile(this.compilationPath, { interval: 2000 }, () => {
  ...
  async _handleRequest(req, res) {
    ...
    // --- Feedback endpoint: user sends note from dashboard ---
    if (req.method === 'POST' && url.pathname === '/api/feedback') {
      ...
      req.on('end', () => {
        try {
          ...
          const fbPath = join(this.dataDir, '.farmer-feedback');
          writeFileSync(fbPath, JSON.stringify({ text, timestamp: Date.now(), read: false }));
          this._audit({ event: 'user_feedback', content: text });
    ...
    // --- Feedback read: Claude marks feedback as consumed ---
    if (req.method === 'POST' && url.pathname === '/api/feedback/read') {
      // Localhost only — called by hooks
      const remoteAddr = req.socket.remoteAddress;
      ...
      try {
        ...
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: true, text: fb.text }));
      } catch {
        res.writeHead(404); res.end('No feedback');
      }
      return;
    }
  ...
  _handlePermission(data, res, req) {
    ...
    if (shouldAutoApprove(session, toolName, toolInput)) {
      let reason = `Auto-approved by ${session.trustLevel} trust level`;
      // Check for unread user feedback — piggyback on hook response
      try {
        const fbPath = join(this.dataDir, '.farmer-feedback');
        const fb = JSON.parse(readFileSync(fbPath, 'utf8'));
        if (fb && !fb.read && fb.text) {
          reason += `\n\n📋 User feedback from Farmer dashboard:\n"${fb.text}"\n(Acknowledge this feedback in your next response to the user. The user sent this from their phone and cannot see CLI output.)`;
          fb.read = true;
          writeFileSync(fbPath, JSON.stringify(fb));
          this._broadcast({ type: 'feedback_read', data: {} });
        }
  ...
  _handleWsUpgrade(req, socket, head) {
    ...
    const GUID = '258EAFA5-E914-47DA-95CA-5AB5AFDC65B2';
  ...
  _loadClaims() {
    ...
    } catch { /* claims file unreadable — ignore */ }
    this._loadCompilation();
  }
  ...
  _loadCompilation() {
    ...
    } catch { /* compilation file unreadable — ignore */ }
  }
  ...
  _initPayload(csrfToken) {
    ...
    return {
      ...
      claims: this.claimsData,
      compilation: this.compilationData,
