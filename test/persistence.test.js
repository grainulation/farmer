...
describe("Persistence", () => {
  ...
  afterEach(() => {
    if (persistence) {
      // Clean up files
      try {
        unlinkSync(persistence.statePath);
      } catch {}
      try {
        unlinkSync(persistence.statePath + ".tmp");
      } catch {}
      try {
        unlinkSync(persistence.auditLogPath);
      } catch {}
    }
  });
  ...
  it("writes audit log entries", () => {
    ...
    const lines = readFileSync(persistence.auditLogPath, "utf8")
      .trim()
      .split("\n");
